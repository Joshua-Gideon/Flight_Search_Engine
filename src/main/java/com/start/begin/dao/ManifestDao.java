...
public interface ManifestDao extends JpaRepository<Manifest, Long> {
	...
	@Query(value="SELECT * FROM manifest WHERE origin=?1 and destination=?2 and stops=?3 and price>=?4 and price<=?5", nativeQuery = true)
	public List<Manifest> findFilghtsWithStops(String origin, String destination, String stops,String  min,String max);
